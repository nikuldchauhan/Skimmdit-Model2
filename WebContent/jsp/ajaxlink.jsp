...
{		 
	...
	Map l = (Map) getServletContext().getAttribute("l");
	Map link = (Map) getServletContext().getAttribute("link");
	Map u = (Map) getServletContext().getAttribute("u");
	if(ty.equals("up"))
	{	
		if(l.containsKey(user))
		{
			...
			if(li.contains(pi))
			{
				...
				linkUser un = (linkUser )link.get(pi);
				int i = un.getVote();
  ...
  Map m = lb.sortedMap(request);
  Map mm = (Map) request.getServletContext().getAttribute("link");
  ...
  while (iter.hasNext())
  {
    ...
    linkUser u = (linkUser) mm.get(entry.getKey());
    Date date = new Date();
	...
	String formattedDate = sdf.format(date);
 	...
 	out.println("<div id='left' style='height:93px;width:40px;float:left;border:1px solid black;'>");
 		out.println("<div id='up' style='height:30px;width:40px;border-bottom:1px solid black;'><img class='vo' ty='up' pi='"+entry.getKey()+"' style='cursor:pointer;' src='images/caret-top-8x.png' width='25px' height='25px' /></div>");
 		out.println("<div id='middle' style='height:30px;width:40px;border-bottom:1px solid black;'>"+u.getVote()+"</div>");
 		out.println("<div id='bottom' style='height:30px;width:40px;'><img class='vo' ty='down' pi='"+entry.getKey()+"' style='cursor:pointer;' src='images/caret-bottom-8x.png' width='25px' height='25px' /></div>");
 	out.println("</div>");
